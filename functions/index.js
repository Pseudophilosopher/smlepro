...
exports.moyasarWebhook = onRequest(
  {
    region: "us-central1",
    secrets: [moyasarSecretKey, SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS, INVOICE_FROM_EMAIL],
  },
  async (req, res) => {
    ...
    try {
      // Verify secret token from Moyasar (if configured)
      const webhookSecret = process.env.WEBHOOK_SECRET || "";
      const authHeader = req.headers.authorization || "";
      
      // Moyasar sends the secret token in the Authorization header as "Bearer {token}"
      ...
      // Log the RAW body as a simple string message so it shows in logs
      const rawBodyStr = typeof req.body === 'object' ? JSON.stringify(req.body) : String(req.body || '');
      console.log("=== MOYASAR RAW BODY START ===");
      console.log(rawBodyStr.substring(0, 3000));
      console.log("=== MOYASAR RAW BODY END ===");
      ...
      if (!userId) {
        ...
        try {
          ...
          if (fullPayment) {
            ...
            // Log the full payment structure to understand where userId might be
            const paymentKeys = Object.keys(fullPayment);
            const hasInvoiceId = !!fullPayment.invoice_id;
            const hasInvoiceObject = !!fullPayment.invoice;
            ...
            if (!userId) {
              ...
              if (uidMatch) {
                ...
                // Last resort: try invoice lookup
                ...
                if (invoicePayment) {
                  ...
                  // Also try description on invoice payment
                  if (!userId) {
      ...
      if (status === "paid") {
        await handleWebhookSuccessfulPayment(paymentId, payment, userId);
      } else if (status === "failed" || status === "canceled") {
        await handleWebhookFailedPayment(paymentId, payment, userId);
...
/**
 * Handle successful payment via webhook - upgrade user to Pro
 */
async function handleWebhookSuccessfulPayment(paymentId, payment, userId) {
  if (!userId) {
  ...
  // TODO: Enable invoice emails when SMTP secrets are configured.
  // For now, Moyasar sends basic receipt emails to customers.
  // sendInvoiceEmail(userId, paymentId, payment);
}

/**
 * Handle failed payment via webhook - log and optionally notify user
 */
async function handleWebhookFailedPayment(paymentId, payment, userId) {
  const paymentRef = db.collection("payments").doc(paymentId);
